...
/// Exact-ranked Compact28 selector. Window 30 is reconstructed from the
/// identity-C outer fold; `FLOCK_NO_R1_WIN30_C_REUSE=1` keeps Compact29 for a
/// same-binary rollback on the materialized A/B path.
#[inline]
fn ranked_win30_c_reuse_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os("FLOCK_NO_R1_WIN30_C_REUSE").is_none()
    });
    *ON
}
...
fn generate_witness_with_ab_packed_and_round1_inner_impl_tuned(
    ...
    let ranked_win30_elided = ranked_compact && ranked_win30_c_reuse_enabled();
    let mut ab_inner = if ranked_win30_elided {
        flock_core::zerocheck::univariate_skip_optimized::Round1AbInner::take_ranked_compact28_uninit(
            n_total * BYTES_PER_BLOCK,
        )
    } else if ranked_compact {
...
fn generate_round1_inner_octa(
    ...
    const COMPACT29_BYTES_PER_BLOCK: usize = 29 * 64;
    const COMPACT28_BYTES_PER_BLOCK: usize = 28 * 64;
    ...
    let win30_elided = ab_inner.ranked_win30_elided();
    let ab_block_bytes = if compact {
        if win30_elided {
            COMPACT28_BYTES_PER_BLOCK
        } else {
            COMPACT29_BYTES_PER_BLOCK
        }
    } else {
        BYTES_PER_BLOCK
    };
