...
fn witgen_urm_share_enabled() -> bool {
    // Default-armed (`FLOCK_NO_WITGEN_URM_SHARE=1` builds the table locally):
    // this archive runs the shared-table arm of the pair experiment — its
    // sibling archive runs witness-local — so the two official draws isolate
    // the table policy on this exact tree.
