...
use flock_core::zerocheck::univariate_skip_optimized::{
    RANKED_WIN30_B_WORD, ROUND1_AB_OFF_WORDS, Round1AbTableImages, Round1AbWindowPlan,
...
pub(crate) struct StreamProj<'t> {
    ...
    /// Compact28 representation: publish ranked z row 30, but omit its AB
    /// projection. Identity-C recovers that fixed-B contribution after bind.
    pub(crate) win30_elided: bool,
}
...
const fn ranked_b30() -> [u8; 64] {
    let mut x = [0u8; 64];
    let w = RANKED_WIN30_B_WORD.to_le_bytes();
    let mut i = 0;
    while i != 8 {
        x[i] = w[i];
        i += 1;
            ...
            for off in (0..words).step_by(STEP_WORDS) {
                ...
                if E && (blk <= 1 || blk == 30 || blk == 31) {
                    ...
                    if blk == 30 {
                        ...
                        if proj.win30_elided {
                            // Keep the materialized witness byte-for-byte:
                            // publish z/a exactly as the Compact29 arm does.
                            // Ranked B's 49/15 tail is pool-provenance static
                            // and this publisher has never rewritten it.
                            // Only the ab_inner projection disappears.
                            let q=proj.stage.cast::<u64>();
                            let mut j=0usize;
