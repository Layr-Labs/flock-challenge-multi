...
impl StreamProj<'_> {
    ...
    unsafe fn project_blocks_ranked_hot_offsets(&self, blk: usize, plan: Round1AbWindowPlan, imgs: Round1AbTableImages, rows: RankedRows, off: *const u16) {
        unsafe {
            ...
            if plan.bcomplement_static_eligible() {
                return self.project_blocks_ranked_hot_offsets_bcomplement_dispatch(blk,plan,imgs,rows,off);
            }
            let (sa,sb)=self.sides();
            let mut j=0usize;
            while j!=8 {
    ...
    #[rustfmt::skip]
    #[inline(never)]
    unsafe fn project_blocks_ranked_hot_offsets_bcomplement_dispatch(&self, blk: usize, plan: Round1AbWindowPlan, imgs: Round1AbTableImages, rows: RankedRows, off: *const u16) {
        unsafe {
            ...
            if *CONSTWIN {
                match blk {
                    3 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<3>(blk,plan,imgs,rows,off),
                    4 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<4>(blk,plan,imgs,rows,off),
                    5 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<5>(blk,plan,imgs,rows,off),
                    6 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<6>(blk,plan,imgs,rows,off),
                    7 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<7>(blk,plan,imgs,rows,off),
                    8 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<8>(blk,plan,imgs,rows,off),
                    9 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<9>(blk,plan,imgs,rows,off),
                    10 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<10>(blk,plan,imgs,rows,off),
                    11 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<11>(blk,plan,imgs,rows,off),
                    12 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<12>(blk,plan,imgs,rows,off),
                    13 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<13>(blk,plan,imgs,rows,off),
                    14 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<14>(blk,plan,imgs,rows,off),
                    15 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<15>(blk,plan,imgs,rows,off),
                    16 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<16>(blk,plan,imgs,rows,off),
                    17 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<17>(blk,plan,imgs,rows,off),
                    18 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<18>(blk,plan,imgs,rows,off),
                    19 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<19>(blk,plan,imgs,rows,off),
                    20 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<20>(blk,plan,imgs,rows,off),
                    21 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<21>(blk,plan,imgs,rows,off),
                    22 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<22>(blk,plan,imgs,rows,off),
                    23 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<23>(blk,plan,imgs,rows,off),
                    24 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<3>(blk,plan,imgs,rows,off),
                    25 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<25>(blk,plan,imgs,rows,off),
                    26 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<5>(blk,plan,imgs,rows,off),
                    27 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<27>(blk,plan,imgs,rows,off),
                    28 => return self.project_blocks_ranked_hot_offsets_bcomplement_const::<28>(blk,plan,imgs,rows,off),
                    _ => {}
                }
            }
    ...
    #[rustfmt::skip]
    #[inline(always)]
    unsafe fn project_blocks_ranked_hot_offsets_bcomplement_const<const BLK: usize>(&self, blk: usize, plan: Round1AbWindowPlan, imgs: Round1AbTableImages, rows: RankedRows, off: *const u16) {
        unsafe {
