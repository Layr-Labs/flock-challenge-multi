...
#[cfg(all(target_feature = "avx512f", target_feature = "avx512bw"))]
#[inline(always)]
unsafe fn ranked_dense_rows_and_offsets(ring: *const V8, w: usize, op: *mut u16) -> [__m512i; 8] {
    unsafe {
...
unsafe fn stage_ranked_dense_side(ring: *const V8, w: usize, stage: *mut u32, op: *mut u16) {
    unsafe {
        let rows = ranked_dense_rows_and_offsets(ring, w, op);
        let mut r = 0usize;
        while r != 8 {
...
impl StreamProj<'_> {
    ...
    #[rustfmt::skip]
    #[inline(never)]
    unsafe fn project_blocks_ranked_hot_offsets_direct(&self, blk: usize, plan: Round1AbWindowPlan, imgs: Round1AbTableImages, rows: RankedRows, a_rows: &[__m512i; 8], b_rows: &[__m512i; 8], off: *const u16) {
        unsafe {
            debug_assert!(blk > 1 && blk < 30);
            if self.one_rows_elided && blk == 2 {
                self.project_blocks_ranked_hot_offsets_residual_direct::<2,0xfc>(plan,imgs,rows,a_rows,b_rows,off);
                return;
            }
            if self.one_rows_elided && blk == 29 {
                self.project_blocks_ranked_hot_offsets_residual_direct::<29,0x0f>(plan,imgs,rows,a_rows,b_rows,off);
                return;
            }
            let mut j=0usize;
            while j!=8 {
    ...
    #[rustfmt::skip]
    #[inline(never)]
    unsafe fn project_blocks_ranked_hot_offsets_residual_direct<const BLK:usize,const KEEP:u8>(&self, plan: Round1AbWindowPlan, imgs: Round1AbTableImages, rows: RankedRows, a_rows: &[__m512i; 8], b_rows: &[__m512i; 8], off: *const u16) {
        unsafe {
            const { assert!((BLK==2 && KEEP==0xfc)||(BLK==29 && KEEP==0x0f)); }
            let mut j=0usize;
            while j!=8 {
                ...
                if E {
                    ...
                    #[cfg(all(target_feature = "avx512f", target_feature = "avx512bw"))]
                    {
                        ...
                        if ranked_direct_dense_publish_enabled() {
                            let a_rows=ranked_dense_rows_and_offsets(self.ast,rw,op);
                            let b_rows=ranked_dense_rows_and_offsets(self.bs,rw,op.add(64));
                            proj.project_blocks_ranked_hot_offsets_direct(blk,plan,imgs,rows,&a_rows,&b_rows,op as *const u16);
                        } else {
    ...
    fn ranked_direct_dense_rows_and_publish_match_staged_path() {
        ...
        unsafe {
            for case in 0..6usize {
                ...
                let mut direct_off = Offsets([0u16; 8 * ROUND1_AB_OFF_WORDS]);
                let a_rows = ranked_dense_rows_and_offsets(
                    a_input.0.as_ptr().cast::<V8>(),
                    0,
                    direct_off.0.as_mut_ptr(),
                );
                let b_rows = ranked_dense_rows_and_offsets(
                    b_input.0.as_ptr().cast::<V8>(),
                    0,
                    direct_off.0.as_mut_ptr().add(64),
                );
                ...
                assert_eq!(direct_off.0, staged_off.0, "offset mismatch, case={case}");
