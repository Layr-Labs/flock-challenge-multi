...
fn ranked_identity_c_fold_enabled(r1cs: &BlockR1cs) -> bool {
    ranked_direct_ab_precompute_enabled(r1cs)
        ...
        && r1cs.k_log >= pcs::LOG_PACKING + 4
        && r1cs.m == 32
        // Tied to the authoritative circuit constants: a future layout change
        // that moves K_LOG/USEFUL_BITS in blake3.rs must move this gate with
        // it, not silently disarm identity-C into the 32-bank drain.
        && r1cs.k_log == crate::r1cs_hashes::blake3::K_LOG
        && r1cs.k_skip == zerocheck::K_SKIP
        && r1cs.useful_bits == crate::r1cs_hashes::blake3::USEFUL_BITS
...
fn prove_fast_ligerito_from_witness_inner<Ch: Challenger>(
    ...
    // Inline, not spawned: the stash is ~tens of µs of work, while the
    // spawn + three deep proof clones + scheduler round-trip + join cost
    // ~180-250 µs on the ranked part per the public record (`b723227`'s
    // family isolation). Same bytes, same fingerprint gate, no thread.
...
fn prove_fast_core_with_codeword_inner<Ch: Challenger>(
    ...
    let ((commitment, prover_data), ab_inner) = if let Some(ab_inner) = ab_inner {
        // The NTT's st_fmp and deep SMT splits arm only when this closure
        // runs OUTSIDE any rayon pool (`current_thread_index() == None`);
        // `in_commit_phase_pool` is the identity off macOS/aarch64 and the
        // caller is the seed-pipe OS thread. Moving this call under a
        // `rayon::join`/`install` would silently disarm both splits.
