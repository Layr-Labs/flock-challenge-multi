...
fn prove_fast_core_with_codeword_inner<Ch: Challenger>(
    ...
    let mut padding = r1cs.padding_spec();
    // `ranked_one_rows_elided` is not a shape guess: only the ranked BLAKE3
    // witness producer brands its challenge-independent AB transform after
    // omitting the circuit's proven constant-one B rows. Carry that existing
    // provenance into round two so its hot kernel can trust the same static B
    // windows without re-validating four ZMMs in every hash block.
    padding.ranked_blake3_bstatic = ab_inner
        .as_ref()
        .is_some_and(zerocheck::univariate_skip_optimized::Round1AbInner::ranked_one_rows_elided);
