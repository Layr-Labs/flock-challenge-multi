...
pub(super) unsafe fn butterfly_fused_2layer_row_from_sparse_dense_geo(
    ...
    unsafe {
        ...
        let inner_low = !diet_disabled
            && !low_inner_sparse_disabled()
            && right_twiddle.hi == 0
            && dense_tw[1].hi == 0
            && dense_tw[2].hi == 0;
        if diet_disabled {
...
mod diet_tests {
    ...
    fn kernels_match_incumbent() {
        ...
        for len in [1usize, 3, 4, 5, 7, 8, 16, 64] {
            ...
            let run_sparse_dense_inner_low = |inner_low: bool| {
                let mut sparse = vec![F128::ZERO; 4 * len];
                let mut dense = vec![F128::ZERO; 4 * len];
                let right_low = F128 { lo: next().lo, hi: 0 };
                let low_tw3_all = [
                    F128 { lo: next().lo, hi: 0 },
                    F128 { lo: next().lo, hi: 0 },
                    F128 { lo: next().lo, hi: 0 },
                ];
                unsafe {
            ...
            assert_eq!(
                run_sparse_dense_inner_low(true),
                run_sparse_dense_inner_low(false),
                "sparse+dense inner-low len={len}"
            );
