...
>(
    ...
    unsafe {
        ...
        while lane + 8 <= lanes {
            ...
            let m_d0 = mul_x4::<false, DIET>(inner_b, vd0);
            let m_d1 = mul_x4::<false, DIET>(inner_b, vd1);
            let new_c0 = _mm512_xor_si512(vc0, m_d0);
            let new_c1 = _mm512_xor_si512(vc1, m_d1);
            vd0 = _mm512_xor_si512(vd0, new_c0);
            vd1 = _mm512_xor_si512(vd1, new_c1);
            vc0 = new_c0;
            vc1 = new_c1;
...
>(
    ...
    unsafe {
        ...
        macro_rules! butterfly2 {
            ($values:ident, $u:expr, $v:expr, $twiddle:expr, $low:expr) => {{
                let m0 = mul_x4::<$low, DIET>($twiddle, $values[0][$v]);
                let m1 = mul_x4::<$low, DIET>($twiddle, $values[1][$v]);
                let new_u0 = _mm512_xor_si512($values[0][$u], m0);
                let new_u1 = _mm512_xor_si512($values[1][$u], m1);
                $values[0][$v] = _mm512_xor_si512($values[0][$v], new_u0);
                $values[1][$v] = _mm512_xor_si512($values[1][$v], new_u1);
                $values[0][$u] = new_u0;
                $values[1][$u] = new_u1;
            }};
        }
        ...
        while lane + 8 <= dense_lanes {
            ...
            let outer = tw[0];
            butterfly2!(values, 0, 4, outer, false);
            butterfly2!(values, 1, 5, outer, false);
            butterfly2!(values, 2, 6, outer, false);
            butterfly2!(values, 3, 7, outer, false);

            let tw1 = tw[1];
            let tw2 = tw[2];
            butterfly2!(values, 0, 2, tw1, LOW_INNER);
            butterfly2!(values, 1, 3, tw1, LOW_INNER);
            butterfly2!(values, 4, 6, tw2, LOW_INNER);
            butterfly2!(values, 5, 7, tw2, LOW_INNER);

            butterfly2!(values, 0, 1, tw[3], LOW_INNER);
            butterfly2!(values, 2, 3, tw[4], LOW_INNER);
            butterfly2!(values, 4, 5, tw[5], LOW_INNER);
            butterfly2!(values, 6, 7, tw[6], LOW_INNER);
