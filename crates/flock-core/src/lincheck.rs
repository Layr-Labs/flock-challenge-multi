...
/// high factor is shared by all lanes and the low factors are one contiguous
/// window, so use the split four-lane multiply rather than loading eight
/// independent high factors.  Other geometries stay on the scalar definition.
fn eq8_from_factors(
    eq_lo: &[F128],
    eq_hi: &[F128],
    outer_base: usize,
    ...
    if lc_eq8_x4_enabled() && log_b >= 3 && outer_base % 8 == 0 {
        // SAFETY: the guard proves the shared-high/contiguous-low geometry;
        // the caller's full-stripe loop supplies the in-range bound.
...
/// Two split 4-lane products of the factorized eq tensor.  At an eight-aligned
/// base and `log_b >= 3`, all eight lanes share `eq_hi[outer_base >> log_b]`
/// while the low factors are contiguous.  `ghash_mul_x4_split` is the same
/// canonical field product as scalar `Mul`, with the shared multiplier's
/// `x^64` companion supplied once.
///
/// # Safety
...
unsafe fn eq8_from_factors_x4(
    ...
    // SAFETY: the dispatcher proves the shared-high geometry; the two loads
    // cover exactly the eight scalar low factors and the stores cover `out`.
    unsafe {
