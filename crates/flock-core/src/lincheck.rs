...
fn eq8_from_factors(
    ...
    if lc_eq8_x4_enabled()
        && outer_base % 8 == 0
        && lo_mask >= 7
        && (lo_mask + 1).is_power_of_two()
    {
        // SAFETY: cfg supplies avx512f+vpclmulqdq; outer_base is 8-aligned and
        // lo_mask is 2^log_b - 1 >= 7.
...
unsafe fn eq8_from_factors_x4(
    ...
    unsafe {
        ...
        let hi_idx = outer_base >> log_b;
        let (t, t_x64) = ghash_broadcast_split(*eq_hi.get_unchecked(hi_idx));
        let lo_ptr = eq_lo.as_ptr().add(lo_base) as *const __m512i;
        let lo0 = _mm512_loadu_si512(lo_ptr);
        let lo1 = _mm512_loadu_si512(lo_ptr.add(1));
        let prod0 = ghash_mul_x4_split(lo0, t, t_x64);
        let prod1 = ghash_mul_x4_split(lo1, t, t_x64);
        let out_ptr = out.as_mut_ptr() as *mut __m512i;
        _mm512_storeu_si512(out_ptr, prod0);
        _mm512_storeu_si512(out_ptr.add(1), prod1);
    }
    out
}
