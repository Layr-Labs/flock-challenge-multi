...
fn blake3_hash_many<const N: usize>(
        ...
        .zip(data.chunks(BLAKE3_BATCH * N))
    {
        ...
        let inputs_ptr = inputs.as_mut_ptr().cast::<&[u8; N]>();
        for i in 0..n {
            unsafe {
        ...
        let inputs_slice: &[&[u8; N]] = unsafe { core::slice::from_raw_parts(inputs_ptr, n) };
        plat.hash_many(
            inputs_slice,
            &BLAKE3_IV,
            0,
...
pub(crate) fn hash_indexed_blake3_1k(data: &[u8], indices: &[usize], out: &mut [Hash]) {
    ...
    out.par_chunks_mut(BLAKE3_BATCH)
        ...
        .for_each(|(outs, positions)| {
            ...
            let inputs_ptr = inputs.as_mut_ptr().cast::<&[u8; LEAF_SIZE]>();
            for i in 0..n {
                unsafe {
        ...
        HashKind::Sha256 => {
            for (outs, children) in write.chunks_mut(4).zip(read_bytes.chunks(256)) {
                if outs.len() == 4 {
                    ...
                    for (out, child) in outs.iter_mut().zip(children.chunks(64)) {
                        *out = Sha256::digest(child).into();
                    }
                }
            }
        }
