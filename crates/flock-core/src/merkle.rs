...
fn blake3_hash_many<const N: usize>(
        ...
        .zip(data.chunks(BLAKE3_BATCH * N))
    {
        ...
        let inputs_slice: &[&[u8; N]] = unsafe {
            core::slice::from_raw_parts(inputs_ptr as *const &[u8; N], n)
        };
...
pub(crate) fn hash_indexed_blake3_1k(data: &[u8], indices: &[usize], out: &mut [Hash]) {
    ...
    out.par_chunks_mut(BLAKE3_BATCH)
        ...
        .for_each(|(outs, positions)| {
            ...
            for i in 0..n {
                // SAFETY: assertions at function entry verify `positions[i] < n_leaves`
                // and `data.len() == n_leaves * LEAF_SIZE`, so each pointer is valid.
                unsafe {
                    inputs_ptr.add(i).write(
                        &*(base_ptr.add(positions[i] * LEAF_SIZE) as *const [u8; LEAF_SIZE]),
                    );
                }
            }
            let inputs_slice: &[&[u8; LEAF_SIZE]] = unsafe {
                core::slice::from_raw_parts(inputs_ptr as *const &[u8; LEAF_SIZE], n)
            };
...
pub(crate) fn hash_leaf_pairs_serial(
    ...
    const SCRATCH_LEAVES: usize = 2 * BLAKE3_BATCH;
