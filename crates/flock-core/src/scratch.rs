...
pub fn prewarm_prover(m: usize) {
    ...
    // addresses. This knob shifts every subsequent allocation by
    // `FLOCK_POOL_PAD_PAGES` 4 KiB pages (touched, leaked leading mapping),
    // a same-binary probe surface for the offset geometry; compiled default
    // 0 = the measured optimum.
    {
    ...
    // Best-effort 2 MiB assembly of every pooled buffer (see
