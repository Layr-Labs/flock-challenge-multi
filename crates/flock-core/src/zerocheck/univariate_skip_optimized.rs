...
fn round1_lifted_from_fold8(
    ...
    static SPARSE_DIRECT: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_R1_C_SPARSE_DIRECT").is_none());
    let quad = if *SPARSE_DIRECT {
        let retained_top_eq = build_eq(&inner_tail[4..6]);
        let retained_hi_eq = build_eq(&inner_tail[2..4]);
        static INIT_SCALE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
            std::env::var_os("FLOCK_NO_R1_C_QUAD_INIT_SCALE").is_none()
        });
        if *INIT_SCALE {
            let scales = [
                retained_top_eq[0] * retained_hi_eq[0],
                retained_top_eq[0] * retained_hi_eq[1],
                retained_top_eq[0] * retained_hi_eq[2],
                retained_top_eq[3] * retained_hi_eq[1],
            ];
            let mut quad = crate::alloc_uninit_vec(4 * n_packed);
            for dst_bank in 0..4 {
                let src = dst_bank * n_packed;
                let dst = dst_bank * n_packed;
                #[cfg(all(target_feature = "avx512f", target_feature = "vpclmulqdq"))]
                // SAFETY: the cfg supplies both features; both slices have
                // exactly `n_packed` elements and do not alias. The kernel
                // writes every destination before any quad read.
                unsafe {
                    crate::pcs::ligerito::eq_expand_block_x4(
                ...
                for (value, &source) in quad[dst..dst + n_packed]
                    .iter_mut()
                    .zip(&fold8[src..src + n_packed])
                {
                    *value = scales[0] * source;
                }
            }
            for (src_bank, dst_bank, scale) in [
                (4usize, 0usize, scales[1]),
                (5, 1, scales[1]),
                (6, 2, scales[1]),
                (7, 3, scales[1]),
                (8, 0, scales[2]),
            ...
            for src_bank in [0usize, 1, 2, 3, 4, 5, 6, 7, 8, 54, 55] {
                let within = src_bank & 15;
                let dst_bank = within & 3;
                let scale = retained_top_eq[src_bank >> 4] * retained_hi_eq[within >> 2];
                let src = src_bank * n_packed;
                let dst = dst_bank * n_packed;
                crate::field::f128_slice::add_scaled(
                    &mut quad[dst..dst + n_packed],
