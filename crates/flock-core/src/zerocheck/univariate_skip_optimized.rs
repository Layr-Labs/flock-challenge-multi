...
fn process_one_x_hi_ab_only(
    ...
    for x_outer_lo in 0..big_lo_size {
        ...
        if eq_fold.is_none() || !ranked_one_rows_elided {
        ...
        if let Some((_, mats, bank_bits)) = eq_fold {
            ...
            if ranked_one_rows_elided {
                ...
                // In the ranked identity-C sweep the transformed rows are
                // contiguous in `ab_inner`; keep the absolute row index for
                // the specialized range-two and full-row kernels.
                let rows_ptr =
