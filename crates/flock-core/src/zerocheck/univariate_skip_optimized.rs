...
/// residual representation may physically pack its 29 live rows while
...
impl Round1AbInner {
    ...
    pub fn take_ranked_compact_uninit(total_bytes: usize) -> Self {
        ...
        let stored_bytes = blocks * RANKED_AB_COMPACT_BLOCK_BYTES;
...
#[inline(always)]
#[allow(unused_variables)]
pub unsafe fn round1_ab_inner_window_from_offsets_nt2_bcomplement_static_const<const BLK: usize>(
    off: &[u16; ROUND1_AB_OFF_WORDS],
    out: &mut [u8; 64],
    plan: Round1AbWindowPlan,
    imgs: Round1AbTableImages,
...
fn process_one_x_hi_ab_only<const DIRECT: bool>(
    ...
    for x_outer_lo in 0..big_lo_size {
        ...
        let live_base = live_row_base(x_outer, first_b_med, ranked_compact);
...
pub fn round1_shift_reduce_ab_packed_padded_with_precomputed(
    ...
    let total_bytes = (1usize << m) / 8;
    assert_eq!(ab_inner.len_bytes(), total_bytes);
    // Compact29 is complete and has no invalid prefix. The exact-ranked
    // stateless caller may omit raw A/B in round one; every other shape and
    // representation retains the full-buffer contract.
    let stateless_compact = ab_inner.ranked_compact()
        && ab_inner.ranked_one_rows_elided()
        && ab_inner.invalid_prefix_bytes() == 0;
    assert_eq!(r.len(), m);
    assert_eq!(inv_table.k, k_skip);
    if stateless_compact && a_packed.is_empty() && b_packed.is_empty() {
        // Nothing to repair: Compact29 was completely produced by witgen.
    } else {
        assert_eq!(a_packed.len(), total_bytes);
        assert_eq!(b_packed.len(), total_bytes);
        ab_inner.fill_invalid_prefix(a_packed, b_packed, inv_table);
