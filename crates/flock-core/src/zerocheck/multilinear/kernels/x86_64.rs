...
///
/// The ordinary two-map leaf first reassembles 64 output `F128`s and a
...
/// rows. Store each plane once and pack its eight-row groups in the same
/// orientation as [`build_row_fold_mats_from_cols`].
///
/// # Safety
...
pub(crate) unsafe fn gfni_fold64_two_maps_to_mats(
    ...
    unsafe {
        ...
        let mut plane_bytes = [0u8; 64];
        for k in 0..16 {
            let value = _mm512_xor_si512(map_plane(&p0, mats0, k), map_plane(&p1, mats1, k));
            _mm512_storeu_si512(plane_bytes.as_mut_ptr().cast::<__m512i>(), value);
            for group in 0..8 {
                let bytes: [u8; 8] =
                    plane_bytes[group * 8..group * 8 + 8].try_into().unwrap();
                out[group * 16 + k] = u64::from_le_bytes(bytes).swap_bytes();
            }
        }
    }
}
