...
fn check_with_fallback(
    ...
    if keep == 0xff {
        let mut preloaded_out = Output([0xa5; 64]);
        // SAFETY: all 64 B bytes are initialized and these offsets were built
        // from exactly this array. The module requires every used CPU feature.
        let preloaded_hit = unsafe {
            let value = _mm512_loadu_si512(b.as_ptr().cast::<__m512i>());
        ...
        assert_eq!(preloaded_hit, expect_hit, "preloaded blk={blk}");
        assert_eq!(preloaded_hit, hit, "the value guard must not be bypassed");
        assert_eq!(preloaded_out.0, out.0, "preloaded/pointer blk={blk}");
        assert_eq!(off.0, saved_offsets, "preloaded input offsets changed");
    }
    if !hit {
...
#[test]
fn disabled_plan_rejects_unreadable_pointer_before_loading_b() {
    let table = table(6, 64);
    let mut out = Output([0xa5; 64]);
    let mut plan = prepare_round1_ab_window_plan(&table, &out.0, true);
    if !plan.offsets_eligible(2) {
        return;
    }
    plan.bcomplement = false;
    let imgs = round1_ab_table_images(&table, plan);
    let (a, b) = input(3, 0x386a_8714);
    let off = offsets(&a, &b);
    let saved = off.0;
    // SAFETY: the disabled-plan contract explicitly declines before reading
    // b_raw. All other arguments are valid initialized storage / table images.
    let hit = unsafe {
        round1_ab_inner_window_from_offsets_nt2_bcomplement(
            &off.0,
            core::ptr::null(),
    ...
    assert_eq!(off.0, saved);
}

#[test]
fn preloaded_entry_rejects_nonwindow_indices_without_writing() {
    let table = table(6, 64);
    let Some(plan) = enabled_plan(&table) else { return };
    let imgs = round1_ab_table_images(&table, plan);
    let (a, b) = input(3, 0x169b_3af1);
    let off = offsets(&a, &b);
    let saved = off.0;
    for blk in [0, 1, 30, 31, 32, usize::MAX] {
        let mut out = Output([0xa5; 64]);
        // SAFETY: full initialized B agrees with every B offset. Only the
        // unsupported index is varied; that index must decline without stores.
        let hit = unsafe {
            let value = _mm512_loadu_si512(b.as_ptr().cast::<__m512i>());
            round1_ab_inner_window_from_offsets_nt2_bcomplement_preloaded(
                &off.0, value, &mut out.0, plan, imgs, blk,
            )
        };
        abinner_publish_fence();
        assert!(!hit, "blk={blk}");
