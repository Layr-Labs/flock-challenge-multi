...
/// `FLOCK_NO_FIXED_AB_EQ_SPLIT=1` restores the general final reduction for
/// same-binary A/B timing. The fixed multiplier is unchanged for the whole
/// conversion call, so the split companion is computed once outside the loop.
#[inline]
fn fixed_ab_eq_split_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_FIXED_AB_EQ_SPLIT").is_none());
    *ON
}
...
pub(crate) unsafe fn accumulate_convert_ab_x86_avx512(
    ...
    use crate::field::gf2_128::x86_64::{f128x4_set, ghash_mul_x4, ghash_mul_x4_split};
    use core::arch::x86_64::*;
    ...
    unsafe {
        ...
        let use_split = fixed_ab_eq_split_enabled();
        for lane in (0..ELL).step_by(4) {
            ...
            let cf = f128x4_set(cf_ab[0], cf_ab[1], cf_ab[2], cf_ab[3]);
            let scaled_ab = if use_split {
                ghash_mul_x4_split(cf, eq, eq_x64)
            } else {
                ghash_mul_x4(cf, eq)
            };
