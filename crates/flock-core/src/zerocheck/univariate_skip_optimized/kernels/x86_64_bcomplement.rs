...
pub(crate) unsafe fn shift_reduce_bcomplement_offw_nt2<const P: bool>(
    ...
    unsafe {
        ...
        let m7 = (modes & 0xff) as u8;
        let av = apply(op.add(offw_krow_words::<P>(7)));
        let mut acc = if m7 == 15 {
            av
        } else if m7 == 0 {
            let bv = apply_b_mode_const::<0, P>(imgs, op.add(64 + offw_krow_words::<P>(7)));
            _mm512_gf2p8mul_epi8(av, bv)
        } else {
            let bv = apply_b_mode_dyn::<P>(imgs, op.add(64 + offw_krow_words::<P>(7)), m7);
            _mm512_xor_si512(_mm512_gf2p8mul_epi8(av, bv), av)
        };
        let x = _mm512_set1_epi8(2);
        for k in (0..7usize).rev() {
            modes >>= 8;
            let mode = (modes & 0xff) as u8;
            let av = apply(op.add(offw_krow_words::<P>(k)));
            let acc_x = _mm512_gf2p8mul_epi8(acc, x);
            if mode == 15 {
                acc = _mm512_xor_si512(acc_x, av);
            } else if mode == 0 {
                let bv = apply_b_mode_const::<0, P>(imgs, op.add(64 + offw_krow_words::<P>(k)));
                let product = _mm512_gf2p8mul_epi8(av, bv);
                acc = _mm512_xor_si512(acc_x, product);
            } else {
                let bv = apply_b_mode_dyn::<P>(imgs, op.add(64 + offw_krow_words::<P>(k)), mode);
                let product = _mm512_gf2p8mul_epi8(av, bv);
...
unsafe fn apply_b_mode_const<const MODE: u8, const P: bool>(
    ...
    unsafe {
        match MODE {
            1 => apply_b_complement::<1, 8, P>(imgs, op),
            2 => apply_b_complement::<2, 8, P>(imgs, op),
            3 => apply_b_complement::<3, 8, P>(imgs, op),
            4 => apply_b_complement::<4, 8, P>(imgs, op),
            5 => apply_b_complement::<5, 8, P>(imgs, op),
            6 => apply_b_complement::<6, 8, P>(imgs, op),
            7 => apply_b_complement::<7, 8, P>(imgs, op),
            8 => apply_b_complement::<0, 7, P>(imgs, op),
            9 => apply_b_complement::<0, 6, P>(imgs, op),
            10 => apply_b_complement::<0, 5, P>(imgs, op),
            11 => apply_b_complement::<0, 4, P>(imgs, op),
            12 => apply_b_complement::<0, 3, P>(imgs, op),
            13 => apply_b_complement::<0, 2, P>(imgs, op),
            14 => apply_b_complement::<0, 1, P>(imgs, op),
            _ => apply_x86_avx512_register_2img_krow_at::<P>(imgs.0, imgs.1, op),
        }
    }
}

#[inline(always)]
unsafe fn apply_b_mode_dyn<const P: bool>(
    imgs: (*const u8, *const u8),
    op: *const u16,
    mode: u8,
) -> __m512i {
    unsafe {
        match mode {
            1 => apply_b_mode_const::<1, P>(imgs, op),
            2 => apply_b_mode_const::<2, P>(imgs, op),
            3 => apply_b_mode_const::<3, P>(imgs, op),
            4 => apply_b_mode_const::<4, P>(imgs, op),
            5 => apply_b_mode_const::<5, P>(imgs, op),
            6 => apply_b_mode_const::<6, P>(imgs, op),
            7 => apply_b_mode_const::<7, P>(imgs, op),
            8 => apply_b_mode_const::<8, P>(imgs, op),
            9 => apply_b_mode_const::<9, P>(imgs, op),
            10 => apply_b_mode_const::<10, P>(imgs, op),
            11 => apply_b_mode_const::<11, P>(imgs, op),
            12 => apply_b_mode_const::<12, P>(imgs, op),
            13 => apply_b_mode_const::<13, P>(imgs, op),
            14 => apply_b_mode_const::<14, P>(imgs, op),
            _ => apply_b_mode_const::<0, P>(imgs, op),
        }
    }
}
...
pub(crate) unsafe fn shift_reduce_bcomplement_offw_nt2_const<const BLK: usize, const P: bool>(
    ...
    let modes = WINDOW_PLANS[BLK - 2].modes;
    let m7 = (modes & 0xff) as u8;
    unsafe {
        let apply = |p| apply_x86_avx512_register_2img_krow_at::<P>(imgs.0, imgs.1, p);
        let av = apply(op.add(offw_krow_words::<P>(7)));
        ...
        macro_rules! step {
            ($k:expr, $shift:expr) => {{
                let mode = ((modes >> $shift) & 0xff) as u8;
                let av = apply(op.add(offw_krow_words::<P>($k)));
                let acc_x = _mm512_gf2p8mul_epi8(acc, x);
                if mode == 15 {
                    acc = _mm512_xor_si512(acc_x, av);
