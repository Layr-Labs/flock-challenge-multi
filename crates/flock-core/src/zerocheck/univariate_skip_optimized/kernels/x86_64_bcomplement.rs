...
/// - On supported (blk, keep), op addresses the original 128-u16 arena (A then
///   B). Every retained K-row has eight readable offsets per operand, each equal
///   to its raw byte << 6. b_raw addresses the corresponding B bytes; retained
///   K-rows are readable and agree with B offsets. Omitted rows need not be
///   initialized. Unsupported shapes return before reading op or b_raw.
...
pub(crate) unsafe fn try_shift_reduce_bcomplement_offw_nt2(
    ...
    // Keep the raw load lazy: unsupported shapes must decline before touching
    // b_raw, and a residual may read only its KEEP-selected qwords.
    unsafe {
        try_shift_reduce_bcomplement_offw_nt2_with_b(
            op,
            |keep| _mm512_maskz_loadu_epi64(keep, b_raw.cast::<i64>()),
            out,
            imgs,
            blk,
            keep,
...
/// Full-window twin for a producer that already holds the actual packed B
/// bytes in a register. This still performs the same byte-value guard; the
/// register is not a certificate that any byte equals one.
///
/// # Safety
/// As for [`try_shift_reduce_bcomplement_offw_nt2`] with KEEP=ff. All 64 bytes
/// in `b_value` must be initialized and match this window's original B offsets.
#[inline(always)]
pub(crate) unsafe fn try_shift_reduce_bcomplement_offw_nt2_preloaded(
    op: *const u16,
    b_value: __m512i,
    out: &mut [u8; 64],
    imgs: (*const u8, *const u8),
    ...
    unsafe { try_shift_reduce_bcomplement_offw_nt2_with_b(op, |_| b_value, out, imgs, blk, 0xff) }
}

/// One guard and arithmetic body for both raw-byte sources. The generic loader
/// introduces no trait-object callback and this body is always-inline. The
/// loader is invoked exactly once, only after (blk, keep) has been accepted.
///
/// # Safety
/// The offset, image and output contracts above apply. On a supported shape,
/// `load_b(keep)` must return the actual packed B bytes for every selected
/// qword; unselected qwords are ignored by the guard and the arithmetic.
#[inline(always)]
unsafe fn try_shift_reduce_bcomplement_offw_nt2_with_b<F>(
    op: *const u16,
    load_b: F,
    out: &mut [u8; 64],
    imgs: (*const u8, *const u8),
    ...
    F: FnOnce(u8) -> __m512i,
{
    ...
    // SAFETY: the caller supplies the actual selected B bytes. The pointer
    // twin loads only KEEP-selected qwords, including for residual windows.
    unsafe {
        let raw = load_b(keep);
...
mod tests {
    ...
    fn rejected_window_shapes_never_write_output() {
        ...
        for blk in [0, 1, 2, 3, 28, 29, 30, 31, 32, usize::MAX] {
            for keep in 0..=u8::MAX {
                ...
                // An invalid shape must not even request the B value. This
                // tests laziness without passing invalid memory to an unsafe
                // function: offsets, images and output all remain legitimate.
                let mut lazy_out = Output([0x5a; 64]);
                let lazy_hit = unsafe {
                    try_shift_reduce_bcomplement_offw_nt2_with_b(
                        off.as_ptr(),
                        |_| panic!("invalid shape requested B: blk={blk} keep={keep:#x}"),
                        &mut lazy_out.0,
                        table.image_ptrs(),
                        blk,
                        keep,
    ...
    #[test]
    fn accepted_loader_receives_keep_once_including_residuals() {
        let table = table(64);
        for (blk, keep) in [(3, 0xff), (2, 0xfc), (29, 0x0f)] {
            let (a, b, off) = window_input(blk);
            let saved = off;
            ...
            let hit = unsafe {
                try_shift_reduce_bcomplement_offw_nt2_with_b(
                    off.as_ptr(),
                    |selected| {
                        assert!(seen.replace(Some(selected)).is_none());
                        _mm512_maskz_loadu_epi64(selected, b.as_ptr().cast::<i64>())
                    },
                    &mut out.0,
                    table.image_ptrs(),
                    blk,
