...
/// Validate one full-window guard shared by eight witness blocks and return
/// the matching immutable mode word. Bit j of all_b_equal_bytes must mean
/// that byte j equals 0xff in ALL eight blocks, not in just one of them.
///
/// The producer can obtain this mask with one byte comparison of the AND of
/// its eight live B staging registers: AND_r B_r[j] == 0xff iff every B_r[j]
/// is 0xff. This function checks only that mask and the window index. Table
/// eligibility and the mask's association with an offset arena remain the
/// checked token constructor's responsibility.
#[inline(always)]
pub(crate) fn bcomplement_octa_modes(blk: usize, all_b_equal_bytes: u64) -> Option<u64> {
    if !(2..=29).contains(&blk) {
        return None;
    }
    let plan = WINDOW_PLANS[blk - 2];
    if all_b_equal_bytes & plan.required != plan.required {
        return None;
    }
    Some(plan.modes)
}

/// Project one member of an already-checked eight-block FULL-window arena.
/// This does not reload raw B or recheck its constant bytes. Keep the original
/// guarded entry point below for callers without the octa token and for the
/// residual KEEP masks.
///
/// # Safety
/// As for [`try_shift_reduce_bcomplement_offw_nt2`] with KEEP=0xff, except
/// there is no b_raw argument: modes must have come from a successful
/// [`bcomplement_octa_modes`] check of the exact eight-block B data from which
/// this arena was built, for the same window. op addresses one of those eight
/// original 128-u16 blocks. The checked token must retain that arena and the
/// same table images; a mode word alone is not evidence about unrelated data.
#[inline(always)]
pub(crate) unsafe fn shift_reduce_bcomplement_prechecked_full_offw_nt2(
    op: *const u16,
    out: &mut [u8; 64],
    imgs: (*const u8, *const u8),
    modes: u64,
) {
    // SAFETY: the token's successful full-window check admits all eight K rows.
    unsafe {
        shift_reduce_bcomplement_prechecked_offw_nt2(op, out, imgs, modes, 7, 0);
    }
}
...
pub(crate) unsafe fn try_shift_reduce_bcomplement_offw_nt2(
    ...
    unsafe {
        ...
        shift_reduce_bcomplement_prechecked_offw_nt2(op, out, imgs, plan.modes, first_k, last_k);
    }
    true
}

/// Shared arithmetic only: the guarded full/residual entry and the checked
/// octa entry use the same projection, Horner order, weighting and NT store.
/// Both callers validate the selected K interval before reaching this helper.
#[inline(always)]
unsafe fn shift_reduce_bcomplement_prechecked_offw_nt2(
    op: *const u16,
    out: &mut [u8; 64],
    imgs: (*const u8, *const u8),
    modes: u64,
    ...
    unsafe {
        let mut modes = modes >> ((7 - first_k) * 8);
...
mod tests {
    ...
    fn octa_input(blk: usize) -> ([[u8; 64]; 8], [[u8; 64]; 8], [u16; 8 * 128]) {
        let (a, b, _) = window_input(blk);
        let mut a_rows = [a; 8];
        let mut b_rows = [b; 8];
        let mut off = [0; 8 * 128];
        for r in 0..8 {
            for byte in 0..64 {
                a_rows[r][byte] ^= (r as u8).wrapping_mul(43);
                if geometry_known_bytes(blk, byte / 8) & (1 << (byte % 8)) == 0 {
                    b_rows[r][byte] ^= (r as u8).wrapping_mul(71);
                }
                off[r * 128 + byte] = u16::from(a_rows[r][byte]) << 6;
                off[r * 128 + 64 + byte] = u16::from(b_rows[r][byte]) << 6;
            }
        }
        (a_rows, b_rows, off)
    }

    /// Scalar oracle for the producer's register AND and final byte compare.
    fn octa_equal_bytes(rows: &[[u8; 64]; 8]) -> u64 {
        let mut equal = 0;
        for byte in 0..64 {
            if rows.iter().fold(0xff, |all, row| all & row[byte]) == 0xff {
                equal |= 1 << byte;
            }
        }
        equal
    ...
    #[test]
    fn octa_mask_checks_every_block_and_only_required_bytes() {
        for blk in 2..=29 {
            let (_, mut b, _) = octa_input(blk);
            let modes = bcomplement_octa_modes(blk, octa_equal_bytes(&b)).unwrap();
            assert_eq!(modes, WINDOW_PLANS[blk - 2].modes);
            // No unrelated byte is required to be 0xff.
            assert_eq!(
                bcomplement_octa_modes(blk, WINDOW_PLANS[blk - 2].required),
                Some(modes)
            );
            for r in 0..8 {
                for byte in 0..64 {
                    let saved = b[r][byte];
                    let required = geometry_known_bytes(blk, byte / 8) & (1 << (byte % 8)) != 0;
                    for bit in 0..8 {
                        b[r][byte] = saved ^ (1 << bit);
                        let got = bcomplement_octa_modes(blk, octa_equal_bytes(&b));
                        assert_eq!(
                            got,
                            if required { None } else { Some(modes) },
                            "blk={blk} block={r} byte={byte} bit={bit}"
                        );
                    }
                    b[r][byte] = saved;
                }
        ...
        for blk in [0, 1, 30, 31, 32, usize::MAX] {
            assert!(bcomplement_octa_modes(blk, u64::MAX).is_none());
        }
    }

    #[test]
    fn prechecked_octa_members_match_guarded_full_windows_and_scalar() {
        for shift in [64, 128, 192] {
            let table = table(shift);
            let imgs = table.image_ptrs();
            for blk in 2..=29 {
                let (a, b, off) = octa_input(blk);
                let saved = off;
                let modes = bcomplement_octa_modes(blk, octa_equal_bytes(&b)).unwrap();
                for r in 0..8 {
                    let mut checked = Output([0xa5; 64]);
                    let mut guarded = Output([0x5a; 64]);
                    // SAFETY: every member's offsets and B bytes were built
                    // together. The aggregate mask checked all eight members;
                    // the mode word and both image pointers stay unchanged.
                    let hit = unsafe {
                        let op = off.as_ptr().add(r * 128);
                        shift_reduce_bcomplement_prechecked_full_offw_nt2(
                            op,
                            &mut checked.0,
                            imgs,
                            modes,
                    ...
                    unsafe { _mm_sfence() };
                    assert!(hit);
                    assert_eq!(checked.0, guarded.0, "blk={blk} block={r}");
                    assert_eq!(checked.0, scalar_window(&table, &a[r], &b[r], 0xff));
                }
                assert_eq!(off, saved);
            }
        }
