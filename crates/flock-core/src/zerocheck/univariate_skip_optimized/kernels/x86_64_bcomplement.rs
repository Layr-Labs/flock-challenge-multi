...
#[inline(always)]
const fn blk_mode(blk: usize, k: usize) -> u8 {
    ((WINDOW_PLANS[blk - 2].modes >> (8 * (7 - k))) & 0xff) as u8
}
...
pub(crate) unsafe fn shift_reduce_bcomplement_offw_nt2_const<const BLK: usize, const P: bool>(
    ...
    unsafe {
        let apply = |p| apply_x86_avx512_register_2img_krow_at::<P>(imgs.0, imgs.1, p);
        let av7 = apply(op.add(offw_krow_words::<P>(7)));
        const MODE7: u8 = blk_mode(BLK, 7);
        let (bv7, correction7) = apply_b_mode_const::<MODE7, P>(
            imgs,
            op.add(64 + offw_krow_words::<P>(7)),
            av7,
        );
        let mut acc = if MODE7 == 15 {
            correction7
        } else if MODE7 == 0 {
            _mm512_gf2p8mul_epi8(av7, bv7)
        } else {
            _mm512_xor_si512(_mm512_gf2p8mul_epi8(av7, bv7), correction7)
        };
        let x = _mm512_set1_epi8(2);
        macro_rules! step {
            ($k:expr) => {{
                const MODE: u8 = blk_mode(BLK, $k);
                let av = apply(op.add(offw_krow_words::<P>($k)));
                ...
                let scaled_acc = _mm512_gf2p8mul_epi8(acc, x);
                acc = if MODE == 15 {
                    _mm512_xor_si512(scaled_acc, correction)
                } else if MODE == 0 {
                    let product = _mm512_gf2p8mul_epi8(av, bv);
                    _mm512_xor_si512(scaled_acc, product)
                } else {
                    let product = _mm512_gf2p8mul_epi8(av, bv);
                    _mm512_ternarylogic_epi64::<0x96>(scaled_acc, product, correction)
                };
            }};
        }
        step!(6);
...
/// Compile-time mode-specialized dispatch inside the const consumer.
#[inline(always)]
