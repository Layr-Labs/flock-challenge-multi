...
pub(crate) unsafe fn shift_reduce_bcomplement_offw_nt2_const<const BLK: usize, const P: bool>(
    ...
    unsafe {
        ...
        const MODE_7: u8 = MODES as u8;
        const MODE_6: u8 = (MODES >> 8) as u8;
        const MODE_5: u8 = (MODES >> 16) as u8;
        const MODE_4: u8 = (MODES >> 24) as u8;
        const MODE_3: u8 = (MODES >> 32) as u8;
        const MODE_2: u8 = (MODES >> 40) as u8;
        const MODE_1: u8 = (MODES >> 48) as u8;
        const MODE_0: u8 = (MODES >> 56) as u8;
        ...
        let mut acc = if MODE_7 == 15 {
            ...
            let bv = apply_x86_avx512_register_2img_krow_at::<P>(
                imgs.0,
                imgs.1,
        ...
        macro_rules! step {
            ($k:expr, $mode:expr) => {{
                let av = apply(op.add(offw_krow_words::<P>($k)));
                if $mode == 15 {
                    acc = _mm512_xor_si512(_mm512_gf2p8mul_epi8(acc, x), av);
                } else if $mode == 0 {
                    let bv = apply_x86_avx512_register_2img_krow_at::<P>(
                        imgs.0,
                        imgs.1,
