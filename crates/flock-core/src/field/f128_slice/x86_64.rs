...
/// Direct 64-bank weighted fold, with four partial sums of ONE final slot
/// in the SIMD lanes: `dst[t] = sum_b w[b] * src[64 * t + b]`.
///
/// Two consecutive loads cover eight banks. A low-limb and a high-limb
/// permute pack adjacent banks into each 128-bit lane, exactly as in the
/// packed weights. Six Karatsuba CLMULs accumulate those eight products;
/// eight groups cover all 64 banks. The lane sums are reduced once and
/// XORed horizontally, so neither a four-slot intermediate nor the old
/// cross-slot transpose is needed. Weights may be arbitrary field values.
///
/// # Safety
/// Caller guarantees `avx512f` + `vpclmulqdq` and exactly 64 readable source
/// elements for every destination element. Source and destination do not
/// overlap. Every iteration stores one complete F128, including short tails.
#[target_feature(enable = "avx512f,vpclmulqdq")]
pub(super) unsafe fn fold64_banked(src: &[F128], dst: &mut [F128], w: &[F128; 64]) {
    use crate::field::gf2_128::x86_64::ghash_reduce_acc_x4;
    use core::arch::x86_64::*;

    debug_assert!(src.len().is_multiple_of(64) && src.len() / 64 == dst.len());
    // SAFETY: the checked wrapper supplies target features and exact bounds.
    unsafe {
        let lo_idx = _mm512_setr_epi64(0, 2, 4, 6, 8, 10, 12, 14);
        let hi_idx = _mm512_setr_epi64(1, 3, 5, 7, 9, 11, 13, 15);
        // Sixteen durable weight vectors, prepared once for the whole
        // block. Lane l owns banks 8*g+2*l and 8*g+2*l+1. Mixed weights
        // stay local to a group instead of adding eight live vectors.
        let wb: [[__m512i; 2]; 8] = core::array::from_fn(|g| {
            let a = _mm512_loadu_si512(w.as_ptr().add(8 * g).cast());
            let b = _mm512_loadu_si512(w.as_ptr().add(8 * g + 4).cast());
            [
                _mm512_permutex2var_epi64(a, lo_idx, b),
                _mm512_permutex2var_epi64(a, hi_idx, b),
            ]
        });
        // One output reads the same 64 F128s as one old fold16 quad.
        // Retain its bounded prefetch distance and existing rollback knob.
        let pf_ahead = fold16_pf_ahead();
        let pf_limit = src.len().saturating_sub(64);
        for (t, value) in dst.iter_mut().enumerate() {
            let base = 64 * t;
            if pf_ahead != 0 && pf_ahead <= pf_limit - base {
                let p = src.as_ptr().add(base + pf_ahead).cast::<i8>();
                let mut line = 0usize;
                while line < 1024 {
            ...
            let mut acc_hi = _mm512_setzero_si512();
            let mut acc_diag = _mm512_setzero_si512();
            for (g, &[w_lo, w_hi]) in wb.iter().enumerate() {
                let a = _mm512_loadu_si512(src.as_ptr().add(base + 8 * g).cast());
                let b = _mm512_loadu_si512(src.as_ptr().add(base + 8 * g + 4).cast());
                let x_lo = _mm512_permutex2var_epi64(a, lo_idx, b);
                let x_hi = _mm512_permutex2var_epi64(a, hi_idx, b);
                acc_lo = _mm512_xor_si512(
                    acc_lo,
                    _mm512_xor_si512(
                        _mm512_clmulepi64_epi128::<0x00>(x_lo, w_lo),
            ...
            let acc_mid = _mm512_xor_si512(acc_diag, _mm512_xor_si512(acc_lo, acc_hi));
            let lanes = ghash_reduce_acc_x4(acc_lo, acc_mid, acc_hi);
            // Reduction is F2-linear. Reducing the four partial sums and
            // then XORing equals reducing the sum of all 64 products.
            let low = _mm_xor_si128(
                _mm512_castsi512_si128(lanes),
                _mm512_extracti32x4_epi32::<1>(lanes),
            );
            let high = _mm_xor_si128(
                _mm512_extracti32x4_epi32::<2>(lanes),
                _mm512_extracti32x4_epi32::<3>(lanes),
            );
            _mm_storeu_si128((value as *mut F128).cast(), _mm_xor_si128(low, high));
        }
    }
}
