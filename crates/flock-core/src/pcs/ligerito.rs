...
fn ranked_open_induce_overlap_enabled(
    ...
    )) && // Default-armed (`FLOCK_NO_LIG_OPEN_INDUCE_OVERLAP=1` restores the
        // separate-phase schedule): the schedule pair on this lineage
        // (`67937882` separate vs `ffd0c34` overlapped, adjacent official
        // draws) read +0.41% for the overlapped arm, and the base lineage
        // ships the overlap armed.
