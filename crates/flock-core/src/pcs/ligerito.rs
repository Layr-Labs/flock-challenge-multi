...
type DirectFold8GfniMats = ([u64; 128], [u64; 128]);
...
#[inline]
fn direct_fold8_gfni_base_mats(generators: &[F128]) -> DirectFold8GfniMats {
    use crate::zerocheck::multilinear::kernels::x86_64::build_row_fold_mats_from_cols;

    assert_eq!(generators.len(), 1 << super::LOG_PACKING);
    (
        build_row_fold_mats_from_cols(&generators[..64]),
        build_row_fold_mats_from_cols(&generators[64..]),
    )
}
...
#[inline]
unsafe fn direct_fold8_gfni_compose_mats(
    base: &DirectFold8GfniMats,
    e_hi: F128,
) -> DirectFold8GfniMats {
    ...
    for i in 0..128 {
        let (lo, hi) = if i < 64 {
            ...
            (&mut lo1[i - 64], &mut hi1[i - 64])
        };
        *lo = power.lo;
        *hi = power.hi;
        power = mul_by_x(power);
    }

    let mut cols0 = [F128::ZERO; 64];
    let mut cols1 = [F128::ZERO; 64];
    // Each row is one power of X times e_hi. Applying the two base half-maps
    // together therefore evaluates M(X^b · e_hi) for 64 columns at a time.
    unsafe {
    ...
    (
        build_row_fold_mats_from_cols(&cols0),
        build_row_fold_mats_from_cols(&cols1),
    )
}
...
fn materialize_direct_fold8_b_gfni_for_precommit(
    ...
    let direct_gfni_base: Vec<DirectFold8GfniMats> = claims
        .par_iter()
        .map(|claim| {
            let generators = direct_fold8_final_generators(claim, challenges[5]);
            direct_fold8_gfni_base_mats(&generators)
        })
        .collect();
    ...
    let stats = folded_b
        ...
        .map_init(
            ...
            |gfni_tmp, (block, (b_out, f_out))| {
                ...
                let (mats0_lo, mats0_hi) = unsafe {
                    direct_fold8_gfni_compose_mats(
                        &direct_gfni_base[0],
                        claim0.eq_hi[block],
                    )
                };
                let (mats1_lo, mats1_hi) = unsafe {
                    direct_fold8_gfni_compose_mats(
                        &direct_gfni_base[1],
                        claim1.eq_hi[block],
                    )
...
fn materialize_direct_fold8(
    ...
    let direct_gfni_base: Vec<DirectFold8GfniMats> = if b_gfni_on {
        claims
            .par_iter()
            .map(|claim| {
                let generators = direct_fold8_final_generators(claim, challenges[5]);
