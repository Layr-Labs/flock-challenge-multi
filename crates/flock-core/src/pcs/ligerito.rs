...
fn induce_densify_local_ntt_fuse_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_LIG_DENSIFY_LOCAL_NTT_FUSE").is_some());
    *ON
}
...
fn transpose_forward_ntt_sparse_inner(
    ...
    let direct_windows = fill && singleton_on && local_ntt_fused && induce_direct_windows_enabled();
...
/// Ranked default folds DirectFold8 f-side 64:1 with one
/// [`crate::field::f128_slice::fold64_banked`] instead of fold16_banked into
/// a 4-wide mid then fold4_nested. Same 64 weighted terms, one deferred
/// reduce instead of four + three pair-muls. `FLOCK_NO_FOLD64_BANKED=1`
/// restores the two-pass form. Default ON. Ranked `env_clear()`.
#[inline]
#[allow(dead_code)] // Selector is consumed only by the ranked AVX-512 route.
fn fold64_banked_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_FOLD64_BANKED").is_none());
    *ON
}
...
fn materialize_direct_fold8_f_for_precommit(
    ...
    let fold64 = fold64_banked_enabled();
    folded_f
        .par_chunks_mut(block_len)
        .enumerate()
        .for_each_init(
            ...
            |mid4, (block, f_out)| {
                ...
                while slot < block_len {
                    let n = SUB.min(block_len - slot);
                    let src = &f_in[64 * slot..64 * (slot + n)];
                    let dst = &mut f_out[slot..slot + n];
                    if fold64 {
                        crate::field::f128_slice::fold64_banked(src, dst, fold64_weight);
                    } else {
                        let m4 = &mut mid4[..4 * n];
                        crate::field::f128_slice::fold16_banked(src, m4, fold16_weight);
                        crate::field::f128_slice::fold4_nested(m4, dst, r4, r5);
                    }
                    slot += n;
                }
            },
...
fn materialize_direct_fold8(
    ...
    let fold64_weight = crate::field::f128_slice::compose_fold64_weights(&fold16_weight, r4, r5);
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    let stats = folded_b
        ...
        .map_init(
            ...
            |(scratch, mid16, mid4, gfni_tmp), (block, (b_out, f_out))| {
                ...
                // Deferred-reduction 64-bank fold (default): 64 -> 1 in one
                // WideGhashX4. Kill switch restores 16-bank fold then nested
                // fold4: 64 -> 4 -> 1.
                let fold64 = fold64_banked_enabled();
                let mut slot = 0usize;
                while slot < block_len {
                    let n = SUB.min(block_len - slot);
                    let m4 = &mut mid4[..4 * n];
                    let f_src = &f_in[64 * slot..64 * (slot + n)];
                    let f_dst = &mut f_out[slot..slot + n];
                    if fold64 {
                        crate::field::f128_slice::fold64_banked(f_src, f_dst, &fold64_weight);
                    } else {
                        crate::field::f128_slice::fold16_banked(f_src, m4, &fold16_weight);
                        crate::field::f128_slice::fold4_nested(m4, f_dst, r4, r5);
                    }
                    if has_ordinary {
