...
fn materialize_direct_fold4(
    ...
    let direct_gfni_maps: Vec<super::ring_switch::GfniDirectFoldMap> = if b_gfni_on {
        direct_tables
            .par_iter()
...
mod tests {
    ...
    fn direct_fold4_two_claim_gfni_matches_composed_table_oracle() {
        ...
        let rows: Vec<(Vec<u64>, Vec<u64>)> = (0..2)
            ...
            .collect();
        let map0 =
            super::super::ring_switch::build_gfni_direct_fold_map_from_table(&direct_tables[0]);
        let map1 =
            super::super::ring_switch::build_gfni_direct_fold_map_from_table(&direct_tables[1]);
        let (mats0_lo, mats0_hi) =
            super::super::ring_switch::compose_block_mats_gfni(&map0, eq_hi[0]);
        let (mats1_lo, mats1_hi) =
            super::super::ring_switch::compose_block_mats_gfni(&map1, eq_hi[1]);
        let mut got = vec![F128::ZERO; 64];
        let mut planes = unsafe { [_mm512_setzero_si512(); 16] };
