...
/// `FLOCK_NO_INDUCE_DIRECT_FILL=1` restores the current two-stage sparse
/// prefix: build one owned buffer per active window, then copy those buffers
/// into the final dense NTT input. Ranked default writes each active window
/// directly into its disjoint final slice while the fused densify pass clears
/// the inactive slices, deleting the intermediate buffers and their copy.
fn induce_direct_fill_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_INDUCE_DIRECT_FILL").is_none());
    *ON
}
...
/// Slice form of [`transpose_forward_ntt_window_dense`]. The direct-fill path
/// owns a disjoint final-data window rather than a temporary `Vec`, but applies
/// the identical layers, twiddles and in-window operand order.
#[inline]
...
/// Materialize one sparse-prefix window directly into its final dense-data
/// slice. `out` is either write-complete via the singleton recurrence or is
/// cleared before the dense collision fallback reads it. `order[rs..re]`
/// retains the established stable per-window accumulation order.
#[allow(clippy::too_many_arguments)]
fn transpose_forward_ntt_sparse_window_into(
    ntt: &AdditiveNttF128,
    positions: &[usize],
    values: &[F128],
    order: &[u32],
    ...
    singleton_on: bool,
    multi_max: usize,
    scratch: &mut Vec<F128>,
    out: &mut [F128],
) {
    debug_assert!(rs < re && re <= order.len());
    ...
    if singleton_on && nnz <= multi_max {
        // The first expansion writes every output slot before reading it.
    ...
    // The dense fallback scatters only the nonzeros, so clear the complete
    // final slice before its first read just as the owned `vec![ZERO; 2^k]`
    // path does.
    out.fill(F128::ZERO);
    for &i in &order[rs..re] {
        let i = i as usize;
        out[positions[i] & wmask] += values[i];
    }
    transpose_forward_ntt_window_dense_in_place(ntt, log_d, k, w, out);
}
...
fn transpose_forward_ntt_sparse_inner(
    ...
    // The ranked fill+fused path does not need an owned buffer per active
    // window: all `2^k` windows already partition the final dense allocation.
    // Attach each stable run to its one final slice, materialize active windows
    // there, and clear inactive slices in the same parallel region. The old
    // processed-buffer path below remains the exact rollback for either gate.
    if fill && induce_fused_densify_enabled() && induce_direct_fill_enabled() {
        let ot = open_timing();
        let mf0 = if ot { minor_faults() } else { 0 };
        let _ta = std::time::Instant::now();
        let nwin = n >> k;
        let mut run_slots: Vec<Option<(usize, usize)>> = vec![None; nwin];
        for &(rs, re) in &runs {
            debug_assert!(rs < re);
            let w = positions[order[rs] as usize] >> k;
            debug_assert!(w < nwin);
            debug_assert!(run_slots[w].is_none(), "window {w} assigned twice");
        ...
        data.par_chunks_mut(1usize << k)
            ...
            .for_each_init(
                Vec::<F128>::new,
                |scratch, (w, (out, run))| match run {
                    Some((rs, re)) => transpose_forward_ntt_sparse_window_into(
                        ntt,
                        positions,
                        values,
                        &order,
                    ...
                    None => out.fill(F128::ZERO),
                },
            );
        let dens_ms = _td.elapsed().as_secs_f64() * 1e3;

        // Remaining steps are unchanged: every final-data slot is initialized
        // before the first dense butterfly reads it.
        let _ts = std::time::Instant::now();
        transpose_forward_ntt_dense_layers(ntt, &mut data, log_d - k);
        if ot {
            eprintln!(
                "      [sparse-ntt] log_d={log_d} k={k} wins={nwin} win-phase 0.00 ms  alloc(final 2^{log_d}) {alloc_ms:.2} ms  direct-fill {dens_ms:.2} ms  dense({dl} layers) {ds:.2} ms  minflt +{mf}",
                nwin = runs.len(),
                dl = log_d - k,
...
/// Sibling-count floor below which the already-computed canonical indices are
/// gathered serially (the parallel gather costs a rayon dispatch; tiny proof
/// vectors do not earn it).
...
fn multi_proof_gather(tree: &[Hash], block_len: usize, queries: &[usize], par: bool) -> Vec<Hash> {
    ...
    if par {
        ...
        // The index walk already fixed the canonical bottom-up emission order.
        // For a small proof, gather those same nodes serially instead of
        // discarding `indices` and repeating the complete sort/dedup + level
        // traversal inside `merkle_multi_proof`.
