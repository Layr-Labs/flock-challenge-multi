...
fn ranked_l0_auth_batch_enabled(
    tree_len: usize, block_len: usize, num_interleaved: usize, n_queries: usize, kind: HashKind,
) -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_L0_AUTH_BATCH").is_none());
    cfg!(all(target_arch = "x86_64", target_feature = "avx512f"))
        && *ON && tree_len == block_len - 1 && block_len == (1 << 20)
        && num_interleaved == 64 && n_queries == 218 && kind == HashKind::Blake3
}
        ...
        rayon::join(
            ...
            || {
                if serial_par_enabled() && stored_indices.len() >= MULTIPROOF_PAR_MIN_SIBLINGS {
                    stored_out.par_iter_mut().zip(stored_indices.par_iter())
