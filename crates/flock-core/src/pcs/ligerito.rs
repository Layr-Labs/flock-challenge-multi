...
fn ranked_open_induce_overlap_enabled(
    ...
    )) && // Opt-in (`FLOCK_LIG_OPEN_INDUCE_OVERLAP=1`): the overlap measured
        // -0.038% in its own lineage's official isolation on the previous
        // crown, and the two highest-scoring third-party trees of 2026-08-29
        // run the separate-phase schedule. Cleared ranked env: separate.
        std::env::var_os("FLOCK_LIG_OPEN_INDUCE_OVERLAP").is_some()
        && direct_fold8_mode
        && log_n == 25
